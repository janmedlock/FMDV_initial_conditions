...
model_a = models.age_structured.Model(t_step=t_step, a_max=a_max)
y_a = model_a.build_initial_conditions()
assert_series_equal(model_a.integral_over_a(y_a), y_u)

model_z = models.time_since_entry_structured.Model(t_step=t_step, z_max=z_max)
y_z = model_z.build_initial_conditions()
assert_series_equal(model_z.integral_over_z(y_z), y_u)

model_c = models.combination.Model(t_step=t_step, a_max=a_max, z_max=z_max)
