...
def _memoize_by_id(func):
    '''Decorator to memoize `func(obj)` using the key `id(obj)`.'''
    cache = {}
...
class Base(_crank_nicolson.Mixin):
    ...
    def __init__(self, solver):
        ...
        self._convert_arr = _memoize_by_id(self._convert_arr_)
    ...
    def _convert_arr_(self, arr, out=None):
        ...
        desired format. This is wrapped by `_memoize_by_id()` to make
        `._convert_arr()`.'''
        if out is not None:
            out[:] = arr[:]
        return arr
    ...
    def M(self, q, y_q, b_mid, out=None):  # pylint: disable=invalid-name
    ...
    def D(self, t_cur, y_cur, y_new):  # pylint: disable=invalid-name
        ...
        M_cur = self.M('cur', y_cur, b_mid)  # pylint: disable=invalid-name
        M_new = self.M('new', y_new, b_mid)  # pylint: disable=invalid-name
    ...
    def __call__(self, t_cur, y_cur, y_new):
        ...
        D = self.D(t_cur, y_cur, y_new)  # pylint: disable=invalid-name
        return self._J(D)
...
class Dense(Base):
    ...
    def _init_temp(self):
        ...
        shape = self.I.shape
        self._temp = self._empty(shape)
        # pylint: disable-next=invalid-name
        self._temp_M = {q: self._empty(shape)
    ...
    def M(self, q, y_q, b_mid, out=None):
        '''Calculate the matrix
        M_q = A_q ± t_step / 2 (b_mid * B
                                + beta @ y_q @ T_q
                                + T_q @ y_q @ beta).'''
        ...
        # self._temp = BT_q is *not* free now.
...
def _get_subclasses(cls):
    '''Generator yielding all of the subclasses of `cls`.'''
    yield cls
    for sub in cls.__subclasses__():
        yield from _get_subclasses(sub)


def Jacobian(solver, method):  # pylint: disable=invalid-name
