...
def run_sat(sat, birth_shape,
    ...
    '''Run one SAT.'''
    model = Model(SAT=sat, birth_shape=birth_shape)
    try:
    ...
    if plot_solution:
        if callable(plot_solution_kwds):
            plot_solution_kwds = plot_solution_kwds(sat, birth_shape)
    ...
    if plot_limit_set:
        if callable(plot_limit_set_kwds):
            plot_limit_set_kwds = plot_limit_set_kwds(sat, birth_shape)
    ...
    if plot_multipliers:
        if callable(plot_multipliers_kwds):
            plot_multipliers_kwds = plot_multipliers_kwds(sat, birth_shape)
        models.plotting.multipliers(multipliers, **plot_multipliers_kwds)


def plot_solution_kwds_build(sats, birth_shapes):
    ...
    def plot_solution_kwds(sat, birth_shape):
        ax = axes[sat]
        legend = (ax.get_subplotspec().is_first_row()
                  & (birth_shape == birth_shapes[0]))
        return {
            'ax': ax,
            'legend': legend,
        }
...
def plot_limit_set_kwds_build(sats, birth_shapes):
    ...
    for state_prop in state_prop_cycle:
        for birth_shape in birth_shapes:
            style = state_prop | _limit_set_style[birth_shape]
    ...
    def plot_limit_set_kwds(sat, birth_shape):
        label = f'SAT{sat}' if birth_shape == birth_shapes[0] else ''
        return {
            'states': PLOT_STATES,
            'ax': ax,
            'label': label,
...
def plot_multipliers_kwds_build(sats, birth_shapes):
    fig = matplotlib.pyplot.figure(
        layout='constrained',
    )
    colors = models.plotting.colors[:len(sats)]
    markers = models.plotting.markers[:len(birth_shapes)]
    ...
    def plot_multipliers_kwds(sat, birth_shape):
        label = f'SAT{sat}, {birth_shape} birth'
        return {
...
def run_sat_kwds_build(sats, birth_shapes,
                       plot_solution=True,
                       plot_limit_set=True,
